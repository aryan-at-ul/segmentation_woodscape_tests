...
class EncNetLoss(nn.CrossEntropyLoss):
    ...
    def __init__(self, aux=False, aux_weight=0.4, weight=None, ignore_index=-1, **kwargs):
        ...
        self.se_loss = True#cfg.MODEL.ENCNET.SE_LOSS
        self.se_weight = 0.2#cfg.MODEL.ENCNET.SE_WEIGHT
        self.nclass = 10
        self.aux = aux
        self.aux_weight = aux_weight
...
class BinaryDiceLoss(nn.Module):
    ...
    def forward(self, predict, target, valid_mask):
        ...
        # print("inside base loss, prediction shape: ", predict.shape)
        # print("inside base loss, target shape: ", target.shape)
...
class DiceLoss(nn.Module):
    ...
    def _base_forward(self, predict, target, valid_mask):
        dice = BinaryDiceLoss(**self.kwargs)
        total_loss = 0
        predict = F.softmax(predict, dim=1)  # Ensure softmax is applied for probability distribution across classes

        # As target is already one-hot encoded, we don't need to one-hot encode it again
        for i in range(predict.shape[1]):  # iterate over the class dimension
            if i != self.ignore_index:
                dice_loss = dice(predict[:, i], target[:, i], valid_mask)
                if self.weight is not None and len(self.weight) > i:
    ...
    def _aux_forward(self, *inputs, **kwargs):
        *preds, target = tuple(inputs)
        valid_mask = (target.sum(dim=1) != 0).long()  # Create a valid mask based on target presence

        loss = 0
        for i, pred in enumerate(preds):
            weight = self.aux_weight if i != 0 else 1  # Apply aux weight only to auxiliary predictions
            loss += self._base_forward(pred, target, valid_mask) * weight
        return loss
